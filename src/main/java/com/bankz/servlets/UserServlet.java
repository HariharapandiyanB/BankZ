...
public class UserServlet extends HttpServlet {
	...
	public void doGet(HttpServletRequest request,HttpServletResponse response) {
		...
		case "/customerFrontPage":
			getCustomerFrontPage(request,response);
			break;
		case "/miniStatement":
...
private void login(HttpServletRequest request,HttpServletResponse response) {
	...
	private void getCustomerFrontPage(HttpServletRequest request,HttpServletResponse response) {
		try {
			request.getRequestDispatcher("/jsp/Customer UI/CustomerFrontPage.jsp").forward(request, response);
	...
	private void miniStatement(HttpServletRequest request,HttpServletResponse response) {
		try {
			...
			do {
				pageNum+=1;
			List<Transaction> transactionsList= logic.getTransactionDetails((int)request.getSession().getAttribute("userId"),pageNum);
	...
	private void transactionDetails(HttpServletRequest request,HttpServletResponse response) {
		try {
			...
			do {
				pageNum+=1;
			List<Transaction> transactionsList= logic.getTransactionDetails(Integer.parseInt(request.getParameter("customerId")),pageNum);
			request.setAttribute("transactionList", transactionsList);
			request.getRequestDispatcher("/jsp/User UI/MiniStatement.jsp").forward(request, response);
			}while(i==1);
		} catch (IOException e) {
	...
	private void getAccountDetails(HttpServletRequest request,HttpServletResponse response) {
		try {
			...
			do {
				pageNum+=1;
				List<Account> accountList=logic.getAccounts((int)request.getSession().getAttribute("userId"), pageNum);
	...
	private void getCustomerAccountDetails(HttpServletRequest request,HttpServletResponse response) {
		try {
			...
			do {
				pageNum+=1;
				List<Account> accountList=logic.getAccounts(Integer.parseInt(request.getParameter("customerId")), pageNum);
				request.setAttribute("accountList", accountList);
				request.getRequestDispatcher("/jsp/User UI/AccountDetails.jsp").forward(request, response);
			}while(i==1);
		}catch (IOException e) {
	...
	private void depositAmount(HttpServletRequest request,HttpServletResponse response) {
		try {
			logic.deposit((int)request.getSession().getAttribute("userId"),Long.parseLong(request.getParameter("accountNumber")),
	...
	private void withdrawAmount(HttpServletRequest request,HttpServletResponse response) {
		try {
			logic.withDrawAmount((int)request.getSession().getAttribute("userId"),Long.parseLong(request.getParameter("accountNumber")),Integer.parseInt(request.getParameter("amount")));
	...
	private void moneyTransfer(HttpServletRequest request,HttpServletResponse response) {
		try {
			logic.moneyTransfer((int)request.getSession().getAttribute("userId"),Integer.parseInt(request.getParameter("amount")) ,
						Long.parseLong(request.getParameter("senderAccountNumber")),Long.parseLong(request.getParameter("receiverAccountNumber")) , request.getParameter("description"));
	...
	private void resetPassword(HttpServletRequest request,HttpServletResponse response) {
		try {
			...
			if (password.equals(logic.login((int)request.getSession().getAttribute("userId"), request.getParameter("oldPassword")).getPassword()) &&
					request.getParameter("1stpassword").equals(request.getParameter("2ndpassword"))) {
				logic.changePassword((int)request.getSession().getAttribute("userId"), request.getParameter("1stpassword"));
	...
	private void getIconPage(HttpServletRequest request,HttpServletResponse response) {
		try {
			...
			if(userType==0|| userType==1) {
				request.setAttribute("employee" ,logic.getEmployeePersonalInfo((int)request.getSession().getAttribute("userId")));
			}else if (userType==2) {
				request.setAttribute("customer" ,logic.getCustomerPersonalInfo((int)request.getSession().getAttribute("userId")));
			}
