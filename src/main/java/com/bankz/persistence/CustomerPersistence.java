...
public class CustomerPersistence extends UserPersistence{
	...
	public void transactionEntry(Account account,int amount,Long secondaryAccNum,long time,String description) 
		...
		Account secondaryAccount=getSingleAccountInfo(secondaryAccNum);
		if(secondaryAccount!=null) {
			List<Object> secondaryRecordValuesList=Arrays.asList(null,secondaryAccount.getCustomerId(),secondaryAccount.getType(),
					amount,secondaryAccount.getBalance(),
					time,account.getAccountNum(),
					secondaryAccount.getAccountNum(),description);
			dbTasks.addRecords("Transaction", dbTasks.fetchColumnList("Transaction"), secondaryRecordValuesList);
